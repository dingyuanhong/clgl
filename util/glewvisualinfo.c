...
VisualInfo (GLContext* ctx, FILE* file)
{
  if (WGLEW_ARB_pixel_format)
    VisualInfoARB(ctx, file,0,0);
  else
    VisualInfoGDI(ctx, file,0,0);
}
...
#  endif
	...
	if (GLEW_OK != err)
	{
		fprintf(file, "Error [main]: glewInit failed: %s\n", glewGetErrorString(err));
		return ;
	}

	fprintf(file, "visual: %d\n", GetPixelFormat(ctx->dc));
	/* ---------------------------------------------------------------------- */
	...
	PrintExtensions(file,(const char*)glGetString(GL_EXTENSIONS));

#ifndef GLEW_NO_GLU
	/* GLU extensions */
	fprintf(file, "GLU version string: %s\n", gluGetString(GLU_VERSION));
	fprintf(file, "GLU extensions (GLU_): \n");
	PrintExtensions(file,(const char*)gluGetString(GLU_EXTENSIONS));
#endif
	...
	if (WGLEW_ARB_extensions_string || WGLEW_EXT_extensions_string)
	{
		...
		PrintExtensions(file,wglGetExtensionsStringARB ?
